"""농업기술 PDF → Embedding → Chroma Vector DB 구축 스크립트.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="농업기술 RAG Vector DB 구축")
    parser.add_argument(
        "--force",
        action="store_true",
        help="캐시/벡터DB를 무시하고 재구축",
    )
    args = parser.parse_args()

    print("=== 임베딩 및 Chroma 저장 ===")
