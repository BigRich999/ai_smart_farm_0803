...
from qwen_client import qwen_generate
...
severity와 confidence는 같게 두고, 근거가 분명할수록 high로 두세요.
"""
...
def _normalize_analysis(result: dict[str, Any], raw: str) -> dict[str, Any]:
    ...
    level = str(
        result.get("severity") or result.get("confidence") or "low"
    ).lower()
...
def _looks_under_diagnosed(analysis: dict[str, Any], user_question: str) -> bool:
    """문제 질문인데 정상·무증상으로만 나온 경우 재분석을 유도한다."""
...
def analyze_crop_image(
    ...
    """이미지를 Qwen3-VL로 분석하여 구조화 결과를 반환한다."""
...
def build_search_query(analysis: dict[str, Any], user_question: str) -> str:
    ...
    # '정상'을 검색어에 넣으면 관련 없는 문서가 섞이므로 제외
    ...
    # 증상/질병이 비어 있으면 작물별 대표 병해 키워드로 보완
